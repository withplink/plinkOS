...
# Collect releases (sorted = chronological by numbered prefix)
releases=()
...
echo ""
echo "Plink Frame v0.1 — Flash Tool"
echo "────────────────────────────────"
for i in "${!releases[@]}"; do
  echo "  $((i+1))) $(basename "${releases[$i]}")"
done
echo ""
read -rp "Select firmware [1-${#releases[@]}]: " choice
...
if [[ ${#ports[@]} -eq 0 ]]; then
  echo "No /dev/cu.usbserial-* found. Connect the UART port (not USB/power) and retry."
  exit 1

...
{
  // Poll the BUSY pin with a 1 ms yield instead of a tight spin. The Spectra 6 waveform holds BUSY
  // for ~31 s; a no-yield spin starves every other task on this core (loop() command draining, and
  // the watchdog). delay(1) hands the CPU back so the render can run on its own FreeRTOS task while
  // loop() keeps servicing BLE commands. 1 ms granularity is irrelevant against a 31 s wait.
  while(!isEPD_W21_BUSY) delay(1);
}

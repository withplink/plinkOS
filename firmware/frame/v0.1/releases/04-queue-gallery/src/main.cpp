...
static void requestRenderPath(const char *path);
...
static void renderTaskFn(void *) {
  for (;;) {
    ...
    for (;;) {
      ...
      if (pending) { strncpy(path, gRenderPath, sizeof(path)); gRenderReqPending = false; }
      portEXIT_CRITICAL(&gRenderReqMux);
      if (!pending) break;
...
void setup() {
  ...
  if (!initSdCard()) {
    ...
    if (items.size() > 0) {
      ...
      requestRenderPath(path);   // async on the render task — see note below
    } else if (SD.exists(kLegacyImage)) {

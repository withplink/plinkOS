...
- `frame.sh` — unified tool: port auto-detect, Flash or Monitor menu; runnable from any root
- `flash.sh` — flash-only variant (still functional standalone)
- Dev `src/main.cpp` retains serial re-render trigger for hot testing
...
- `frame.sh` — new unified flash+monitor tool
- `flash.sh` — monitor-after-flash prompt moved pre-flash; bash 3.2 compat fix
- `README.md` — updated Stack section (GxEPD2 → GooDisplay native driver), added releases/flash.sh entries

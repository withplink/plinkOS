  ...
  switch (c.op) {
    ...
    case kBleIdentify: {
      ...
      if (idx < 0) {
        ...
        // Mismatch (reinstall, or a revoked-then-restored mapping) — block. Only clear OUR
        // app-identity bookkeeping, not the BLE bond: this identify arrived over an already-
        // encrypted link (peer.isEncrypted()==1 at this point), meaning the LTK just used is
        // valid and current on BOTH sides right now. Deleting it here (as a prior version did)
        // destroyed that live, correct key — the phone kept trusting it (it just negotiated
        // those keys seconds ago) while the frame no longer had a copy, so every reconnect after
        // failed with "Peer removed pairing information" (code=14) and no self-recovery was
        // possible short of forgetting the device in iOS Settings again. Confirmed on hardware
        // (2026-08-15): isBonded flips 1→0 across that deleteBond call, and the very next
        // connection attempt fails auth (encrypted=0) even with the pairing window still open —
        // proving the window wasn't the blocker, this was.

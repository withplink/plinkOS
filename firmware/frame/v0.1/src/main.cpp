...
// Persisted framebuffer for re-render without SD/BMP re-decode.
// Freed only on power cycle. Set after first successful render.
...
static bool renderBmpFromSd(const char *path) {
  ...
  if (kFrameRotation != 0) {
    ...
    // physFb[ly][lx] = fb[(EPD_WIDTH-1-lx)][ly]
